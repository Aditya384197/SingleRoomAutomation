...
// 🔥 FIX (security, minor): constant-time comparison. String::equals() short-
// circuits on the first mismatched byte, which in principle leaks timing
// information an attacker could use to guess the key byte-by-byte. This walks
// the full length regardless of where the mismatch is.
static bool constantTimeEquals(const String &a, const char *b) {
...
static bool checkAuth(AsyncWebServerRequest *request) {
  ...
  return constantTimeEquals(request->getHeader("X-API-Key")->value(), API_SECRET_KEY);
}
