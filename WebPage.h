...
<body>
    ...
    <script>
        ...
        // 🔥 FIX (security): WiFi SSID is attacker-controllable text (anyone
        // nearby can name their network anything, including HTML/JS). It was
        // being inserted directly into innerHTML unescaped - a malicious SSID
        // like '"><img src=x onerror=alert(1)>' would execute in the browser
        // viewing the dashboard, with access to the API key. Escape before
        // ever putting untrusted text into innerHTML.
        ...
        function pollWifiScan(attempt) {
            ...
            apiGet('/api/wifiscan', function (resp) {
                ...
                for (var i = 0; i < resp.networks.length; i++) {
                    ...
                    var safeSsid = escapeHtml(net.ssid);
                    html += '<div class="wifi-item" data-ssid="' + safeSsid + '" onclick="connectWiFi(this)">' +
                        safeSsid + '<span class="signal">' + signalBars(net.rssi) + '</span></div>';
                }
                list.innerHTML = html;
            }, function () {
                statusText.textContent = 'Scan failed.';
